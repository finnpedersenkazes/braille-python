...
def charToDots(char):
    return louis.charToDots(translationList(), char, len(char))
...
def textToDots(text):
    ...
    ordDotsArray = list(map(ord, dotsArray))
    printProperty('ordDotsArray', str(ordDotsArray))

    adjustedDotsArray = list(map(adjustNumber, ordDotsArray))
    printProperty('adjustedDotsArray', str(adjustedDotsArray))

    cells = []
    for i in range(0, 40): # it must be the length of the display
        if i < len(adjustedDotsArray):
            cells.append(adjustedDotsArray[i])
        else:
            cells.append(0)
...
def helper(m):
    ...
    for _ in range(20, 40): # it must be the length of the display
        cells.append(0)
...
def view(brl, m):
    ...
    if (m['counter'] == 0) or m['stop']:
        brl.writeDots(textToDots(m['message']))
        # brl.writeText()
    else:
